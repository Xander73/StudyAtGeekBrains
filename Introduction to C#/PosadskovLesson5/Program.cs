...
namespace PosadskovLesson5
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            DirectoryTreeLoop(@"E:\0_work\Study\StudyAtGeekBrains\PosadskovLesson5");
        ...
        static void DirectoryTreeLoop(string p)
        {
            ...
            else
            {
                ...
                DirectoryInfo prevCatalog = path;
                while (catalogTree.Count != 0)
                {
                    if (catalogTree.Peek() is DirectoryInfo d)
                    {
                        if (!d.FullName.Equals(prevCatalog.FullName))
                        {
                            ...
                            {
                                PrintCatalog((DirectoryInfo)catalogTree.Pop());
                            }
                        }
                        else
                        {
                            PrintCatalog((DirectoryInfo)catalogTree.Pop());
                            if (catalogTree.Count == 0)
                            {
                                break;
                            }
                           prevCatalog = prevCatalog.Parent;
                        }
                    }
                    //subPath = path.GetFiles();
        ...
        {
            StringBuilder resultate = default;
            var subPath = path.GetFiles();
            resultate = new StringBuilder('\\' + path.Name + resultate);
            for (int i = 0; i < subPath.Length; i++)
            {
                resultate = new StringBuilder('\t' + subPath[i].Name + resultate);
            }
